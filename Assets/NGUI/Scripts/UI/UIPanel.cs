...
public class UIPanel : UIRect
{
	...
	public int clipCount {
		get {
			...
			while (p != null) {
				if (p.mClipping == UIDrawCall.Clipping.SoftClip || p.mClipping == UIDrawCall.Clipping.TextureMask)
					++count;
				p = p.mParentPanel;
			}
			return count;
	...
	void UpdateDrawCalls()
	{
		...
		if (drawCallClipRange.z == 0f)
			drawCallClipRange.z = Screen.width * 0.5f;
		if (drawCallClipRange.w == 0f)
			drawCallClipRange.w = Screen.height * 0.5f;
	...
	Vector2 GetWindowSize()
	{
		...
		if (rt != null)
			size *= rt.GetPixelSizeAdjustment(Mathf.RoundToInt(size.y));
		return size;
	}
	...
	void OnDrawGizmos()
	{
		...
		Vector3[] corners = worldCorners;
		for (int i = 0; i < 4; ++i)
			corners [i] = t.InverseTransformPoint(corners [i]);
